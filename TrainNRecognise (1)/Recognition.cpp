...
int recognition(){
	...
	dataOut.open("comingsAndGoings.csv");//todo make dynamic
	dataOut << "write test success";
	string imgPath = getImgPath();
	vector<vector<string>> fileNames = getFilename(imgPath);//gets a 2d vector array of all image files at the path
	vector<string> fullFile;
	for (int x = 0; x != (fileNames.size()); ++x) {
		fullFile.push_back(imgPath + "/" + fileNames.at(x).at(0));
	}
	...
	for (int x = 0; x != fullFile.size(); ++x) {
		...
		if (blnKNNTrainingSuccessful == false) {                            // if KNN training was not successful
																			// show error message
			cout << endl << endl << "Error: KNN traning was not successful, please restart the program" << endl << endl;
			system("pause");
		...
		if (imgOriginalScene.empty()) {                             // if unable to open image
			...
			if (vectorOfPossiblePlates.empty()) {                                               // if no plates were found
				...
				if (licPlate.strChars.length() == 0) {                                                      // if no chars were found in the plate
					...
					string pltPath = imgPath + "(plates)" + "/" + fileNames.at(x).at(0);

					fs::path path_target(pltPath);
					fs::path path_folder = path_target.parent_path();//extract   folder
					if (!fs::exists(path_folder)) //create folder if it doesn't exist
					{
						fs::create_directory(path_folder);
					}
...
vector<PossiblePlate> detectCharsInPlates(vector<PossiblePlate> &vectorOfPossiblePlates) {
	...
	for (auto &possiblePlate : vectorOfPossiblePlates) {            // for each possible plate, this is a big for loop that takes up most of the function
		...
		threshold(possiblePlate.imgThresh, possiblePlate.imgThresh, 0.0, 255.0, cv::THRESH_BINARY | cv::THRESH_OTSU);
...
vector<PossibleChar> findPossibleCharsInPlate(Mat &imgGrayscale, Mat &imgThresh) {
	...
	findContours(imgThreshCopy, contours, cv::RETR_LIST, cv::CHAIN_APPROX_SIMPLE);        // find all contours in plate
...
string recognizeCharsInPlate(Mat &imgThresh, vector<PossibleChar> &vectorOfMatchingChars) {
	...
	cvtColor(imgThresh, imgThreshColor, cv::COLOR_GRAY2BGR);       // make color version of threshold image so we can draw contours in color on it
...
vector<PossibleChar> findPossibleCharsInScene(Mat &imgThresh) {
	...
	findContours(imgThreshCopy, contours, cv::RETR_LIST, cv::CHAIN_APPROX_SIMPLE);        // find all contours
...
void preprocess(Mat &imgOriginal, Mat &imgGrayscale, Mat &imgThresh) {
	...
	adaptiveThreshold(imgBlurred, imgThresh, 255.0, cv::ADAPTIVE_THRESH_GAUSSIAN_C, cv::THRESH_BINARY_INV, ADAPTIVE_THRESH_BLOCK_SIZE, ADAPTIVE_THRESH_WEIGHT);
}
...
Mat maximizeContrast(Mat &imgGrayscale) {
	...
	morphologyEx(imgGrayscale, imgTopHat, MORPH_TOPHAT, structuringElement);//tophat = src - open(source, element))
...
Mat extractValue(Mat &imgOriginal) {
	...
	cvtColor(imgOriginal, imgHSV, cv::COLOR_BGR2GRAY);
